...
import {
  BetaProvider,
  useBetaContext,
} from "../../../contexts/betaContext";
import { createBetaConfig } from "../../../utils/betaConfig";
import { 
...
interface ScoreType {
  // Define your score structure for LaiveBeta if needed
}
...
export default function LaiveBetaScreen() {
  return (
    <BetaProvider>
      <InterviewFlow />
    </BetaProvider>
...
function InterviewFlow() {
  ...
  const handleStartInterview = (name: string, role: string, lang: LanguagePref) => {
...
function WelcomeScreen({
  ...
  onStart: (shortName: string, roleApply: string, languagePref: LanguagePref) => void;
}) {
      ...
      <View style={styles.welcomeLayout}>
        <View style={styles.welcomeBranding}>
          ...
          <Text style={[styles.welcomeSubtitle, { color: theme.colors.onSurfaceVariant }]}>
            Welcome to your AI-powered screening. Please provide the following details to begin.
          </Text>
        </View>
        ...
        <View style={styles.welcomeForm}>
          <Card style={{width: '100%', padding: 12}}>
            ...
            <View style={styles.languageSelector}>
              <Text style={[styles.languageLabel, { color: theme.colors.onSurface }]}>
                Select Role:
              </Text>
              <RadioButton.Group
                ...
                value={roleApply!}
              >
                <RadioButton.Item label="Customer Service Agent" value="Customer Service Agent" />
              </RadioButton.Group>
            </View>
            <View style={styles.languageSelector}>
              <Text style={[styles.languageLabel, { color: theme.colors.onSurface }]}>
              ...
              <RadioButton.Group
                onValueChange={(newValue) => setLanguagePref(newValue as LanguagePref)}
                value={languagePref}
              >
                <View style={styles.radioRow}>
                  <RadioButton.Item label="English" value="English" />
                  <RadioButton.Item label="Bahasa Malaysia" value="Bahasa Malaysia" />
                </View>
                <View style={styles.radioRow}>
...
function PreparationScreen({ onProceed, onBack }: { onProceed: () => void; onBack: () => void; }) {
  const theme = useTheme();
  const tips = [
    {
...
function MessageBubble({ item, userName }: { item: ItemType; userName: string; }) {
  const theme = useTheme();
...
function ReportScreen({ onRestart, shortName }: { onRestart: () => void; shortName: string; }) {
  const theme = useTheme();
      ...
      <View style={styles.reportLayout}>
        <View style={styles.reportMessage}>
          <Avatar.Icon
            ...
            color={theme.colors.onPrimary}
          />
          <Text style={[styles.reportTitle, { color: theme.colors.onBackground }]}>
            Thank You, {shortName}!
          </Text>
          <Text
            style={[
          ...
          >
            We appreciate you taking the time to complete the screening. The hiring team will review your results and be in touch shortly.
          </Text>
        </View>
        <View style={styles.reportActions}>
          <Card style={[styles.reportCard, { backgroundColor: theme.colors.surface, width: '100%', maxWidth: 400 }]}>
            <Card.Content>
              <Text style={{fontSize: 18, fontWeight: 'bold', textAlign: 'center', color: theme.colors.onSurface, marginBottom: 20}}>
                Download Your Results
              </Text>
              <View style={{flexDirection: 'row', justifyContent: 'space-around'}}>
                <Button icon="file-pdf-box" mode="contained" onPress={() => handleDownload('pdf')}>
                  PDF
                </Button>
                <Button icon="file-delimited" mode="contained" onPress={() => handleDownload('csv')}>
                  CSV
                </Button>
              </View>
            </Card.Content>
...
const styles = StyleSheet.create({
  ...
  welcomeTitle: { fontSize: 36, fontWeight: "bold", textAlign: "center", marginBottom: 12 },
  welcomeSubtitle: {
    fontSize: 18,
    textAlign: "center",
    maxWidth: 400,
